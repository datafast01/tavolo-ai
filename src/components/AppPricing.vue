<template>
  ...
  <div>
    ...
    <div class="d-flex align-center justify-center mx-auto py-10">
      ...
      <div class="position-relative">
        <div class="pricing-save-chip position-absolute d-sm-block d-none">
          <VIcon start icon="mdi-arrow-down-left" size="24" class="text-disabled flip-in-rtl mt-1" />
    ...
    <VRow v-if="!annualMonthlyPlanPriceToggler">
      <VCol v-for="(plan, index) in monthlyPkgs" :key="index" v-bind="props" cols="12" md="4">
        <!-- 👉  Card -->
        <VCard flat border class="border-primary border-opacity-100">
          ...
          <VCardText class="text-center">
            <VImg :height="120" :src="pricingPlans[index].logo" class="mx-auto mb-5" />
          ...
          <VCardText class="pt-2">
            <VList class="card-list">
              <VListItem v-for="feature in pricingPlans[index].features" :key="feature">
                <template #prepend>
          ...
          <!-- 👉 Plan actions -->
          <VCardActions>
            <VBtn block :color="plan._id === currentPkgId ? 'success' : 'primary'"
              :variant="plan._id !== currentPkgId ? 'elevated' : 'tonal'" @click="checkout(plan)">
    ...
    <VRow v-else>
      <VCol v-for="(plan, index) in yearlyPkgs" :key="index" v-bind="props" cols="12" md="4">
        <!-- 👉  Card -->
        <VCard flat border class="border-primary border-opacity-100">
          <VCardText style="block-size: 4.125rem" class="text-end">
            <!-- 👉 Popular -->
          ...
          <VCardActions>
            <VBtn block :color="plan._id === currentPkgId ? 'success' : 'primary'"
              :variant="plan._id != currentPkgId ? 'elevated' : 'tonal'" @click="checkout(plan, currentPkgId)">
              {{ plan._id === currentPkgId ? "Your Current Plan" : "Upgrade" }}
            </VBtn>
          </VCardActions>
        </VCard>
